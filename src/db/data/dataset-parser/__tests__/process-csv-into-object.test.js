...
describe('Process CSV into object script', () => {
  ...
  it('should call process-row-into-object on each row', () => {
    ...
    const file = { path: 'file-path', year: 2019, numberUnitsAreMillions: true, usePresupuestadoAsOriginal: false, filename: 'filename.csv' };
    ...
    expect(mockParseRow.mock.calls[0][0]).toEqual({row: 'mock-row', numberUnitsAreMillions: true, usePresupuestadoAsOriginal: false, inflation: 1, filename: 'filename.csv'});
    expect(mockProcessRowIntoObject).toHaveBeenCalledTimes(1);
    expect(mockProcessRowIntoObject.mock.calls).toEqual([[{row: 'parsed-row', dbObject}]]);
  });
});